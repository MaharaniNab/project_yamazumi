<div
    ...
    <div class="p-6 space-y-4">
        <div class="flex justify-between items-center">
            ...
            <div class="flex items-center gap-4">
                ...
                {{-- BUTTON --}}
                @can('create_role')
                    <flux:button icon="plus" wire:click="createRole"
                            ...
                            <div class="flex justify-center items-center gap-2">
                                @can('update_role')
                                    <flux:modal.trigger name="edit-role-{{ $role->id }}">
                                        <flux:button icon="pencil-square" icon:variant="solid" tooltip="Edit Role"
                                            tooltip:position="top" wire:click="edit({{ $role->id }})" size="sm"
                                            variant="primary" color="blue" />
                                    </flux:modal.trigger>
                                @endcan
                                @can('delete_role')
                                    <flux:button icon="trash" icon:variant="solid" tooltip="Hapus Role" tooltip:position="top"
                                        wire:click="confirmDelete({{ $role->id }})" size="sm" variant="danger" />
                                @endcan
                            </div>
                        </flux:table.cell>
                    </flux:table.row>
    ...
    : 'Buat role baru dan atur permission.' }}
            ...
            {{-- PERMISSIONS --}}
            <div class="rounded-xl border border-gray-200 dark:border-zinc-700 p-4 space-y-4">
                ...
                <ul class="space-y-3 text-sm">
                    @foreach ($menus as $menu)
                        <li class="p-3 rounded-lg border border-gray-100 dark:border-zinc-800 bg-gray-50 dark:bg-zinc-900">
                            ...
                            <flux:checkbox.group wire:model="selectedPermissions">
                                ...
                                @if (!empty($menu['permissions']))
                                    <div class="pl-6 space-y-2 text-neutral-700 dark:text-neutral-300">
                                ...
                                @if (!empty($menu['children']))
                                    <div class="mt-4 pl-4 space-y-4 border-l border-gray-200 dark:border-zinc-700">
            ...
            <div class="mt-8 pt-5 border-t border-gray-200 dark:border-zinc-700">
                <div class="flex justify-end gap-3">
                    ...
                    <flux:button type="submit" variant="primary" wire:loading.attr="disabled"
                        class="px-5 py-2 rounded-lg font-semibold shadow-md hover:shadow-lg transition-all duration-200">
